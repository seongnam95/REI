...
from PySide6.QtCore import Qt, QSize, QRegularExpression, QRect, QEvent
from PySide6.QtGui import QIcon, QRegularExpressionValidator, QFont, QColor, QGuiApplication
from urllib3.connectionpool import xrange

from ui.custom.BlackBoxMsg import BoxMessage
from interface.public import get_address
from interface.contract.resources.ui.ui_contract import Ui_Contract
from interface.contract.sub import editor


class MinContract(QMainWindow, Ui_Contract):
    def __init__(self, *args, **kwargs):
        super(MinContract, self).__init__(*args, **kwargs)
    ...
    def clicked_address_edit(self, e):
        dialog = get_address.AddressDetails(0, self.edt_address.text())
        dialog.exec()
    ...
    def load_title(self):
        ...
        if self.cbx_category.currentIndex() == 0:
            ...
            title = self.agr[self.agr['category'] == category]
            title = title.sort_values(['titleNum'], ascending=True)
            title = title.reset_index(drop=True)
            title = list(dict.fromkeys(title['title']))
    ...
    def load_content(self):
        ...
        content = self.agr.content[(self.agr['category'] == category) & (self.agr['title'] == title)]
        self.edt_agrs.setHtml(content.iloc[0])

        self.edt_agrs.setFocus()

    # 특약사항 편집 이벤트
    def clicked_edit_btn(self):
        dialog = editor.Editor(self.agr, self.user_pk)
        dialog.exec()
    ...
    def eventFilter(self, source, event):
        ...
        return super(MinContract, self).eventFilter(source, event)
