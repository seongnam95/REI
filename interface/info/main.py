...
import data.sys_data as sys_data
import module.mysql as sql
import module.bot.set_chrome as set_chrome
...
from interface.info.sub import issuance_register, issuance_ledger
from interface.public import get_address
from ui.custom import BlackBoxMsg, MenuWidget, LoadingBox
from interface.info.resources.ui.ui_info import UiInfo


class Info(QMainWindow, UiInfo):
    def __init__(self):
        super(Info, self).__init__()

        # 시스템 데이터에 SQL 데이터 삽입
        sys_data.api_key = sql.get_api_key()

        self.BULIDING_API_KEY = sys_data.api_key['data']
        self.VIOL_KEY = sys_data.api_key['viol']
        ...
        # 세움터 로그인
        self.issuance_thread = set_chrome.SetChrome('haul1115', 'ks05090818@')
    ...
    def _init_ui(self):
        ...
        # 메뉴 위젯 설정
        items = [{'name': '내 매물로 등록', 'img': 'resources/img/plus_square_icon.png'},
                 {'name': '계약서 작성', 'img': 'resources/img/pen_icon.png'},
                 {'name': '광고 업로드', 'img': 'resources/img/upload_icon.png'},
                 {'name': '매물 공유하기', 'img': 'resources/img/share_icon.png'},
                 {'name': '설정', 'img': 'resources/img/setting_icon.png'}]
        ...
        self.btn_menu.setIcon(QIcon('resources/img/menu_icon.png'))
        self.btn_menu.setIconSize(QSize(20, 20))
        self.btn_issuance.setIcon(QIcon('resources/img/print_icon.png'))
